...
from models.enums import CritiqueVerdict, DocumentStatus, GlossaryScope, Language, TaskType
...
class EnrichedContext(BaseModel):
    ...
    model_config = {"extra": "forbid"}  # Ensures additionalProperties: false at top level

    detected_intent: str = Field(..., description="Detected user intent")
    document_title: str = Field(default="", description="A natural, professional title for the document being generated (e.g. 'Clinical Summary for Spiriva Respimat')")
    ...
    input_summary: str = Field(default="", description="Summarised version of the user input")
...
class AdaptedContentSection(BaseModel):
    ...
    section_id: str = Field(..., description="The template placeholder or chapter ID this content belongs to (e.g. 'definitions_abbreviations')",)
    section_title: str = Field(...,description="The proper human-readable chapter heading exactly as it appears in the template (e.g. 'DEFINITIONS & ABBREVIATIONS')",)
    content: str = Field(..., description="The generated Markdown content adapted for this section")
    original_excerpt: str = Field(default="", description="Relevant text extracted from the old document")
    changes_made: str = Field(default="", description="Summary of syntactic or structural changes made to the original text")
    reasoning: str = Field(default="", description="Why this content was generated or modified in this way")


class AdaptationMap(BaseModel):
    """Full mapping output containing all adapted sections for the document."""
    sections: list[AdaptedContentSection] = Field(..., description="List of all adapted sections")
    overall_mapping_strategy: str = Field(default="", description="High-level strategy used for mapping the old document to the new template")
